...
public abstract class Group {
	...
	/**
	 * Returns the order of the group (ie, the number of elements in it)
	 * 
	 * @return the order
	 */
	 ...
	 * called, <i>n</i> is greater than 0. Subclasses may wish to override this
	 * if they have a more efficient exponentiation implementation. However,
	 * beware of <a href="http://en.wikipedia.org/wiki/Side_channel_attack">
	...
	/**
	 * Groups are expected to be parameterized by one or more integer values
	 * (e.g. the modulus for groups of integers mod <i>m</i>). This methods
	 * returns a parameterization of this particular instance of the group.
	 * 
	 ...
	 * </pre>
	 * 
	 * where <i>classname</i> is the canonical name of the Java class
	 * implementing the group, and <i>n1</i>, ..., <i>nk</i> are integer
	 * parameters. Note that the semicolon will still be present in the output
	 * even if <i>k</i>=0.
	 * 
	 * @return
	 */
	...
	/**
	 * Reconstruct a group from its String representation. This is the reason
	 * for the highly-specific {@link #toString} format.
	 * 
	 * @param s
	...
	public static Group fromString(String s) {
		...
		if (s.length() >= 2) {
			...
			if (nameAndParams.length == 2) {
				...
				// Parse the parameters into BigIntegers, to pass to the
				// constructor
				...
				// There are many ways this could fail; if it does, just log the
				// error
		...
		if (error != null) {
			Log.d(Group.class.getSimpleName(), "Could not instantiate group '"
					+ s + "'.  Reason:\n", error);
		}
	 ...
	 * 
	 * @param s
	 *            A string representation, as returned by the toString method of
	 *            an element belonging to this group
