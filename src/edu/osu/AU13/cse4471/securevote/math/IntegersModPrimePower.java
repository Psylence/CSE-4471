...
/**
 * An extension of IntegersModM for representing a more specific type of modulo group
 * 
 * @author Alex
 */
public class IntegersModPrimePower extends IntegersModM {
	...
	/**
	 * Creates a random prime and power and returns the prime power group that represents them
	 * @param primeSize The max size of the prime in bits
	 * @param powMax The max value for the power
	 * @return A new IntegersModPrimePower group
	 */
	public static IntegersModPrimePower generateRandom(int primeSize, int powMax) {
		if(powMax < 1) throw new IllegalArgumentException("powMax must be strictly positive");
		
		SecureRandom rng = new SecureRandom();
		
		BigInteger prime = BigInteger.probablePrime(primeSize, rng); // How convenient!
	...
	public IntegersModPrimePower(BigInteger prime, int pow) {
		...
		// The universe is more likely to end before this returns a false positive for PRIME_TEST = 500
		if(!prime.isProbablePrime(PRIME_TEST)) throw new IllegalArgumentException("The prime provided is actually composite.");
