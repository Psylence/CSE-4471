...
public class EnterEmailActivity extends Activity {
	...
	private void doDialog(final int editIndex) {
		...
		if (editIndex >= 0) {
			alert.setTitle(getResources().getString(
					R.string.email_edit_dialog_title));
		} else {
			alert.setTitle(getResources().getString(
					R.string.email_add_dialog_title));
		}
		alert.setMessage(getResources().getString(
				R.string.email_add_dialog_message));
		...
		alert.setPositiveButton(android.R.string.ok,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (email != null) {
							EnterEmailActivity.setOrAdd(editIndex, mAdapter,
									email);
						} else {
	...
	/**
	 * Return a valid email (without leading whitespace), or null if the input
	 * is not a salvageable email address. Currently does only the barest
